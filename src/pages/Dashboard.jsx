...
const QuickActionCard = ({ title, count, icon: Icon, accent, to, index }) => (
  ...
  >
    <Link
      to={to}
      className="block bg-white rounded-xl p-4 border border-[#E6D5C3] shadow-card hover:shadow-card-hover hover:border-[#2E3192] transition-all duration-200 group"
    >
      ...
      <div className="mt-3 flex items-center text-xs font-medium text-[#2E3192] group-hover:text-[#1E2163] transition-colors">
        View details
...
const ActivityItem = ({ item }) => {
  ...
  const getActionIcon = () => {
    if (isAdminLog) {
      ...
      if (item.action.includes('property')) return <Building2 className="w-4 h-4 text-[#2E3192]" />;
    }
  ...
  const getActionColor = () => {
    if (isAdminLog) {
      ...
      if (item.action.includes('property')) return "bg-[#2E3192]/10";
    }

    return isProperty ? "bg-[#2E3192]/10" : "bg-blue-50";
  };
  ...
  const statCards = [
    {
      ...
      icon: Home,
      accent: { bg: "bg-[#2E3192]/10", icon: "text-[#2E3192]" },
    ...
    datasets: [
      {
        ...
        backgroundColor: ["#2E3192", "#E6D5C3"],
        borderColor: ["#1E2163", "#D4B99A"],
        borderWidth: 1,
        hoverOffset: 6,
      },
    ],
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F4] pt-8">
        <div className="text-center max-w-md">
          ...
          <button onClick={() => fetchStats()}
            className="px-6 py-3 bg-[#2E3192] text-white rounded-xl font-semibold text-sm hover:bg-[#1E2163] transition-colors">
            Try Again
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto pt-8">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="flex items-center gap-2 px-4 py-2.5 bg-white border border-[#E6D5C3] text-[#1C1B1A] rounded-xl text-sm font-medium hover:border-[#2E3192] hover:text-[#2E3192] transition-all duration-200 shadow-card disabled:opacity-60 disabled:cursor-not-allowed"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-1.5 text-xs text-[#2E3192] font-medium">
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <Activity className="w-5 h-5 text-[#2E3192]" />
              <h3 className="text-base font-bold text-[#1C1B1A]">Recent Admin Activity</h3>
            </div>
            <Link
              to="/activity-logs"
              className="text-xs text-[#2E3192] hover:text-[#1E2163] font-medium transition-colors"
            >
              View all
            </Link>
          </div>
