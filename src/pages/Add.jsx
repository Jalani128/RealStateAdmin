...
const inputClass = "w-full px-4 py-3 bg-white border border-[#E6D5C3] rounded-xl text-[#1C1B1A] placeholder-[#9CA3AF] text-sm transition-all duration-200 outline-none focus:border-[#2E3192] focus:ring-2 focus:ring-[#2E3192]/15";
const labelClass = "block text-sm font-semibold text-[#1C1B1A] mb-2";

const SectionHeader = ({ icon: Icon, title, subtitle }) => (
  <div className="flex items-center gap-3 mb-5">
    <div className="w-9 h-9 bg-[#2E3192]/10 rounded-xl flex items-center justify-center">
      <Icon className="w-4.5 h-4.5 text-[#2E3192]" />
    </div>
    <div>
    ...
    <div className="min-h-screen pt-8 pb-12 px-4 bg-[#FAF8F4]">
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <p className="text-[#5A5856]">Fill in the details to list a new property on RealEstate</p>
        </motion.div>
        ...
        >
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#E6D5C3] shadow-card">
                  ...
                  <button key={amenity} type="button" onClick={() => handleAmenityToggle(amenity)}
                    className={cn(
                      ...
                      selected
                        ...
                        : 'bg-[#FAF8F4] text-[#5A5856] border border-[#E6D5C3] hover:border-[#2E3192] hover:text-[#2E3192]'
                    )}
                  >
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={handleAddAmenity}
                className="flex items-center gap-1.5 px-4 py-2.5 bg-[#1C1B1A] text-white rounded-xl text-sm font-medium hover:bg-[#2E3192] transition-colors">
            ...
            {formData.amenities.filter((a) => !AMENITIES_LIST.includes(a)).length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {formData.amenities.filter((a) => !AMENITIES_LIST.includes(a)).map((amenity) => (
                  <span key={amenity}
                    className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#2E3192]/10 text-[#2E3192] rounded-full text-sm font-medium">
                    {amenity}
                    <button type="button" onClick={() => handleAmenityToggle(amenity)}
                      className="hover:text-[#1E2163] transition-colors">
                      <X size={13} />
                    </button>
                  </span>
                ))}
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#E6D5C3] shadow-card">
            ...
            {previewUrls.length < 4 && (
              <label htmlFor="images"
                className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-[#E6D5C3] rounded-xl cursor-pointer bg-[#FAF8F4] hover:border-[#2E3192] hover:bg-[#2E3192]/5 transition-all duration-200 group">
                <Upload className="w-8 h-8 text-[#9CA3AF] group-hover:text-[#2E3192] mb-2 transition-colors" />
                <span className="text-sm font-medium text-[#5A5856] group-hover:text-[#2E3192] transition-colors">
          ...
          <motion.button
            ...
            className="w-full py-4 bg-[#1C1B1A] hover:bg-[#2E3192] text-[#FAF8F4] rounded-xl font-semibold text-base transition-all duration-300 shadow-lg hover:shadow-terracotta disabled:opacity-60 disabled:cursor-not-allowed"
          >
